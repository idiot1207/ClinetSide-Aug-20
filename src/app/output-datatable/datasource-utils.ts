...
export function sortRows<U>(
  ...
  console.log('woww sortRows working');
  return (rows$: Observable<U[]>) => combineLatest(
    rows$,
    sort$.pipe(toSortFn(sortFns, useDefault)),
    (rows, sortFn) => {
      if (!sortFn) { console.log('iniside sortRows'); return rows; }
      console.log('iniside sortRows');
...
export function vinoth(){
  console.log('inisde vinayak Mahadev');
}
